...
function shortenContact(contact: Record<string, string>): Record<string, string> {
  ...
  if (email && (email.length > 12 || !email.includes("@"))) {
    const local =
      email.split("@")[0]?.replace(/\W/g, "").slice(0, 2) || "u";
...
export async function generatePhase1TestData(
  ...
  try {
    ...
    const raw = {
      company: (parsed.company ?? parsed.Company) as
        | Record<string, string>
        | undefined,
      contact: (parsed.contact ?? parsed.Contact) as
        | Record<string, string>
        | undefined,
    };
    ...
    if (!raw.company || typeof raw.company !== "object") {
      const e = new Error(
        "Gemini response missing company (keys: " +
    ...
    if (!raw.contact || typeof raw.contact !== "object") {
      const e = new Error(
        "Gemini response missing contact (keys: " +
          Object.keys(parsed).join(", ") +
          ")",
      ) as Error & { debugText?: string };
      e.debugText = debugText;
    ...
    if (
      ...
      typeof err === "object" &&
      "debugText" in err &&
      typeof (err as { debugText?: string }).debugText === "string"
    ) {
      console.warn(
        "Gemini raw response (first 500 chars):",
        (err as { debugText: string }).debugText.slice(0, 500),
      );
    }
    const fallback =
      FALLBACK_DATA[Math.floor(Math.random() * FALLBACK_DATA.length)];
    return {
      company: ensureMandatoryCompanyFields(
        { ...fallback.company },
        fallback.company.country === "Germany"
          ? "Germany"
          : "Switzerland",
      ),
      contact: {
...
export function logGeminiStatus(): void {
  ...
  if (!key) {
    console.log(
      "[Gemini] No API key (GEMINI_API_KEY/GOOGLE_API_KEY); using static fallback.",
    );
    return;
  }
  console.log(
    "[Gemini] API key set; will attempt to generate company/contact data. On failure, static DE/CH fallback is used.",
  );
}
