...
function token(): string {
  ...
  if (!t)
    throw new Error(
      "HUBSPOT_ACCESS_TOKEN is not set (or HUBSPOT_ACCESS_TOKEN_SANDBOX when HUBSPOT_ENV=sandbox)",
    );
  return t;
}
...
// ---------------------------------------------------------------------------
// Re-export all existing helpers (copied from previous deal-invoice-qa/hubspot.ts)
// ---------------------------------------------------------------------------
...
export async function getSelectedQuoteId(dealId: string): Promise<string> {
  ...
  console.log(
    `  Selected quote ${chosen[0].id} (sign_status: ${
      chosen[0].hs_sign_status ?? "none"
    }, hs_status: ${chosen[0].hs_status})`,
  );
  return chosen[0].id;
}
...
export async function getCalculatedQuoteTotal(
  ...
  if (lineItemIds.length > 0) {
    ...
    for (const item of liBatch.results ?? []) {
      ...
      const lineDiscount = parseFloat(
        item.properties.hs_total_discount ?? "0",
      );
  ...
  console.log(
    `  Quote ${quoteId}: lineItemsSubtotal=${subtotalBeforeDiscounts.toFixed(
      2,
    )}, afterDiscounts(net)=${rounded}`,
  );
  return rounded;
}
...
export async function getAllInvoicesForDeal(
  ...
  console.log(
    `  Found ${invoices.length} invoice(s): ${invoices
      .map((i) => `${i.id} net=${i.subtotal} gross=${i.grossTotal}`)
      .join(", ")}`,
  );
  return invoices;
}
