/**
 * Phase 1 CRM schema and constants.
 * Property names and pipeline IDs for Company, Contact, Deal.
 * Populate pipeline/stage IDs via portal audit (GET /crm/v3/properties, GET /crm/v3/pipelines).
 */

// ---------------------------------------------------------------------------
// Company (Phase 1) – 14 mandatory fields
// ---------------------------------------------------------------------------

/** All 14 mandatory company properties for creation. */
export const COMPANY_MANDATORY_FIELDS = [
  "name",
  "address",
  "city",
  "zip",
  "country",
  "original_country",
  "industry",
  "status",
  "company_number",
  "company_code",
  "customer_account_group",
  "phone",
  "website",
  "e_mail",
] as const;

export const COMPANY_PROPERTIES = [...COMPANY_MANDATORY_FIELDS] as const;

export type CompanyPropertyName = (typeof COMPANY_PROPERTIES)[number];

/** Required for creation. Omit status to let WF 1007 set "Aktiv". */
export const COMPANY_REQUIRED_FOR_CREATE: readonly string[] = [
  ...COMPANY_MANDATORY_FIELDS,
];

/** HubSpot company industry is an enum. Use one of these (e.g. MARKETING_AND_ADVERTISING). */
export const COMPANY_INDUSTRY_EXAMPLE = "MARKETING_AND_ADVERTISING";

/** Map country name to HubSpot original_country (and possibly country) enum code. */
export const COUNTRY_TO_CODE: Record<string, string> = {
  Switzerland: "CH",
  Germany: "DE",
};

// ---------------------------------------------------------------------------
// Contact (Phase 1)
// ---------------------------------------------------------------------------

export const CONTACT_PROPERTIES = [
  "firstname",
  "lastname",
  "email",
  "hs_lifecyclestage",
  "hs_language",
  "jobtitle",
] as const;

export type ContactPropertyName = (typeof CONTACT_PROPERTIES)[number];

/** Required for creation. */
export const CONTACT_REQUIRED_FOR_CREATE: readonly string[] = [
  "firstname",
  "lastname",
  "email",
];

// ---------------------------------------------------------------------------
// Deal (Phase 2 handover – documented here for reference)
// ---------------------------------------------------------------------------

/** Order pipeline ID (from E2E plan). */
export const ORDER_PIPELINE_ID = "3387257046";

/** Test gate: WF 1040 sets when HubSpot Team = B4U. Verify internal name in portal (e.g. test_record_b4you or test_gate). */
export const DEAL_PROPERTY_TEST_GATE = "test_record_b4you";

/** Deal property names that Phase 2 depends on (line item sync + quote sync). */
export const PHASE2_DEAL_PROPERTIES: readonly string[] = [
  "dealname",
  "pipeline",
  "dealstage",
  "billing_cycle",
  "tax_rate",
  "deal_currency_code",
  "product_group",
  "revenue_stream",
  "line_item_true",
] as const;

/** Line item properties that Phase 2 assertions depend on. */
export const PHASE2_LINE_ITEM_PROPERTIES: readonly string[] = [
  "name",
  "quantity",
  "price",
  "amount",
  "description",
  "product_group",
  "spot_duration",
  "spot_count",
  "plattform",
  "time_category",
  "week_type",
  "laufzeit",
  "kw",
  "jv_amount_of_digital_ads_per_second",
  "hs_total_discount",
  "original_line_item_id",
] as const;

/** Internal status labels for key pipeline stages used in Phase 2. */
export const PHASE2_DEAL_STATUS_LABELS = {
  salesInProgress: "sales_in_progress",
  quoteReady: "quote_ready",
  salesClosedWon: "sales_closed_won",
  orderClarification: "order_clarification",
  orderConfirmed: "order_confirmed",
} as const;

// ---------------------------------------------------------------------------
// Associations
// ---------------------------------------------------------------------------

/** Contact to Company: HubSpot default association type ID. Verify via GET /crm/v4/associations/contact/company/labels */
export const ASSOCIATION_CONTACT_TO_COMPANY_TYPE_ID = 1;

// ---------------------------------------------------------------------------
// Golden Data Record (Phase 1 output for Phase 2)
// ---------------------------------------------------------------------------

export interface Phase1CompanySnapshot {
  status: string;
  name: string;
  country: string;
}

export interface Phase1ContactSnapshot {
  lifecycle_stage: string | null;
  email: string;
}

export interface Phase1GoldenRecord {
  companyId: string;
  contactId: string;
  company: Phase1CompanySnapshot;
  contact: Phase1ContactSnapshot;
}
