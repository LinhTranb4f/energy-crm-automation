...
export async function calculateRadioSpots(
  props: LineItemProps,
): Promise<CalculationResult> {
  const { tableId } = HUBDB.radioSpots;
  const filters: Record<string, string> = {};
  for (const col of HUBDB.radioSpots.filterColumns) {
    const val = props[col];
  ...
  if (rows.length === 0) {
    console.warn(
      `[Radio Spots] No HubDB match for filters: ${JSON.stringify(filters)}`,
    );
    return {
      expected_price: 0,
